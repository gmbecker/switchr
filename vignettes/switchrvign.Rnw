...
We can create a package manifest from scratch via the \texttt{PkgManifest} function, though there
...
package. This uses Csardi's the database of R package metadata
developed for his
...
process. This is achieved via the \texttt{switchrDontUnload} function,
which sets a persistent option telling \verb@switchr@ to ignore those
packages when emptying the current R session. Base packages,
\verb@switchr@ itself, and its dependencies are always ignored in this way.
